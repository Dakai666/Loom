...
|---------|------|-----------|
| **v0.4.1.0** | 2026-09-17 | **Prediction Spine retired** — after three months of observation the automatic layers (implicit per-tool bets, weekly reconcile, calibration residue + immune system, P1 `environment_friction` affect arm) were removed: >99% of the corpus was hard-coded heartbeat bets narrated back to the agent as its own beliefs, and no decision was ever changed by it. The explicit `predict` tool stays and now settles in-session, with HIT / MISS appended to the target tool's result (#580); design history archived in `docs/retired/prediction-spine/`. Kept from the spine work: semantic observation surface — `action_records` output capture + `output_contains` / `output_regex` / `row_count` resolvers (#569). Circadian: daily weave's global layer delivered at dawn (#565/#567), `circadian:weave_applied` event (#472/#562), actionable schema-validation errors + JSON-text array coercion (#566). Memory: consolidation dream backlog drain + sturdier diff-inventory (#553/#554), content-addressed compress fact keys (#550), compression yield alert removed + unread-batch fix (#581). Discord forum publishing: `create_discord_forum_post` + `read_discord_forum` (#549). MCP stdio teardown noise muted (#572) |
