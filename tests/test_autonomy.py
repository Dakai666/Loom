...
"""
    ...
    async def test_execute_plan_safe_calls_agent(self):
        ...
        # Use a plain dataclass-style object with a `text` attribute so the
        # daemon's hasattr check picks it up without importing platform types.
        class _Text:
        ...
        # Verify stream_turn was called with the right prompt
        assert calls == ["do something"]

        # A REPORT notification should be sent with the streamed output
        report_notifs = [n for n in received if n.type == NotificationType.REPORT]
    ...
    async def test_execute_plan_skip_does_not_call_agent(self):
        ...
        async def mock_stream(prompt):  # pragma: no cover
            calls.append(prompt)
            return
