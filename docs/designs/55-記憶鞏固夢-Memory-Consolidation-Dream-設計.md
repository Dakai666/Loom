...
> **2026-09-22 修訂（#587）**：09-20 首跑實證這條二分規則也擋下了「同一主張的不同面向」（42 簇 approve 4 / skip 16，veto 多半來自「各自攜帶他人所無的內容」）。gate 改為四分類：`duplicate`（同義反覆）/ `extends`（同一核心主張、細節互相相容 → 以「核心句＋細節聯集」合併，每條細節逐條保留並標日期與來源，漏一條就拒絕合併）/ `distinct`（不同主張，含不同日期的快照）/ `conflict`（成員互相矛盾 → 跳過，報告列為需仲裁）。上面「偏好 vs 抱怨」的例子歸 `distinct`，仍然不合併。
